...
{
	TRACE("hasnext:%d isdire:%d name:%s\r\n", Info.HasNext, Info.IsDirectory, Info.szFileName);
	if (Info.HasNext == FALSE) return;
	//TRACE("<name>%s   <isdir>%d\r\n", pInfo->szFileName, pInfo->IsDirectory);
	if (Info.IsDirectory) {
		if ((CString(Info.szFileName) == ".") || (CString(Info.szFileName) == "..")) {
			return;
		}
		//TRACE("hselected %08X\r\n",lParam);
		HTREEITEM hTemp = m_tree.InsertItem(Info.szFileName, (HTREEITEM)hlParam);
		m_tree.InsertItem("", hTemp, TVI_LAST);
		m_tree.Expand((HTREEITEM)hlParam, TVE_EXPAND);
	}
	else
	{
		m_List.InsertItem(0, Info.szFileName);
	}
}
...
void CRemoteClientDlg::DealCommand(WORD nCmd, const std::string& strData, LPARAM lParam)
{
	...
	case 2://获取文件信息
		...
		break;
	case 3:
		MessageBox("文件打开成功", "操作完成", MB_ICONINFORMATION);
		break;
	case 4:
		UpdateDownloadFile(strData, (FILE*)lParam);  //必须以string类型传参，以char*传参时会遇到'\0'结束
		break;
	case 9:
		MessageBox("文件删除成功", "操作完成", MB_ICONINFORMATION);
		break;
	case 1981:
		MessageBox("连接测试成功", "成功", MB_ICONINFORMATION);
		break;
	default:
...
LRESULT CRemoteClientDlg::OnSendPackAck(WPARAM wParam, LPARAM lParam)
{
	if ((lParam == -1) || (lParam == -2)) {//出错
		TRACE("socket is wrong %d\r\n", lParam);
	}
	else if (lParam == 1) {
		//接收完毕,对方关闭了套接字或者网络设备异常
		TRACE("socket has closed\r\n");
	}
	else{//正常接收		
		if (wParam != NULL) {
